        ...
        <div className="flex flex-col items-center">
          ...
          <div className="qrCube relative w-[440px] h-[440px] border-2 border-gray-300 rounded-lg grid grid-cols-3 grid-rows-3 gap-1">
            {piecesState.map((piece, index) => (
              <div
                key={`${piece.image}-${index}`} // Clé unique pour forcer le re-rendu
                  ...
                  piece.link ? (
                    <>
                      <Image
                        ...
                        alt={`Grid ${index + 1}`}
                        fill
                        className={`rounded-md object-cover transition-opacity duration-1000 opacity-0 ${index !== reversedMilestones.length - 1 && !piece.reached ?  'opacity-100' :'group-[&:hover]:opacity-100'}`}
                        sizes="(max-width: 440px) 33vw, 146px"
                      />
                      <div
                        className={`absolute inset-0 bg-opacity-60 opacity-0 group-hover:opacity-100 flex items-end justify-center p-2 transition-opacity duration-300`}
                    ...
                    <>
                      <Image
                        ...
                        className={`rounded-md object-cover transition-opacity duration-1000 opacity-0 ${index !== reversedMilestones.length - 1 && !piece.reached ?  'opacity-100' :'group-[&:hover]:opacity-100'}`}
                        sizes="(max-width: 440px) 33vw, 146px"
                        onLoadingComplete={(img) => img.classList.add("opacity-100")} // Transition après chargement
                      />
