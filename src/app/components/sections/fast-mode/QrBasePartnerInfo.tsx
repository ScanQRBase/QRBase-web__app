...
export default function QrBasePartnerInfo({ partnerData, scanData, scanBalance, partnerBalance, currentRound, nftBalance }: any) {
  ...
  const getStatus = (nft: any) => {
    if (nft === null) return 'unknown';
    return nft.length > 0 ? 'accepted' : 'rejected';
  };
  ...
  return (
    <div className="flex flex-col justify-center border-gray-200 border-b p-4 py-8 pb-12 md:w-1/3 md:border-r md:border-b-0 md:py-22 lg:border-r lg:p-6 lg:pb-22 md:pt-8">

      <div className="space-y-4 text-left parTnerInfoHeightFastMode">
            ...
            <div className="flex items-center space-x-2">
              <p className="text-[0.75rem] leading-relaxed" style={{ display: 'flex', flexDirection: 'column' }}>
                ...
                <div className="flex items-center space-x-2" style={{ marginTop: '10px', marginBottom: '5px' }}>
                  ...
                  <StatusIcon status={getStatus(nftBalance)} />
                  <span
                    onClick={() => router.push("/x402/boostpass")}
                    className="flex cursor-pointer items-center hover:underline"


                  >
                    <p className="text-[0.75rem] leading-relaxed font-bold">BoostPass NFT</p>
                    <span className="pl-1">
                      <ExternalLinkSvg />
                    </span>
                  </span>
